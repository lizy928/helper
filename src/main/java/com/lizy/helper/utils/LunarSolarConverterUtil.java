...
public class LunarSolarConverterUtil {
    ...
    public static Date lunarToDate(Date lunarDate) {
        LocalDateTime localDateTime = LocalDateTime.ofInstant(lunarDate.toInstant(), TimeZone.getDefault().toZoneId());
        Lunar l = new Lunar();
        l.isleap = false;
        l.lunarDay = localDateTime.getDayOfMonth();
        l.lunarMonth = localDateTime.getMonth().getValue();
        l.lunarYear = localDateTime.getYear();
        final int hours = localDateTime.getHour();
        final int minutes = localDateTime.getMinute();
        final int seconds = localDateTime.getSecond();

        final Solar solar = lunarToSolar(l);

        final LocalDateTime dateTime = LocalDateTime.of(solar.solarYear, solar.solarMonth, solar.solarDay, hours, minutes, seconds);
     ...
     * @return
     */
    public static EventReminderModel getTimeDiff(Date target) {
        // 输入指定日期
        long timeMillis = target.getTime();
        ...
        EventReminderModel eventReminder = new EventReminderModel();
        eventReminder.setDay(day);
        eventReminder.setHour(hour);
        eventReminder.setMinute(minute);
        eventReminder.setSecond(second);
//        final String format = String.format("%s天%s时%s分%s秒", day, hour, minute, second);
...
//        l.lunarYear = 2021;
//
//        final Solar solar1 = lunarToSolar(l);
//        System.out.println(solar1);

        Date date = new Date();
        date.setDate(30);

        System.out.println(getTimeDiff(date));

//        Lunar lunar = solarToLunar(solar);
//        System.out.println(dump(lunar));
//        solar = LunarSolarConverterUtil.lunarToSolar(l);
//        System.out.println(dump(solar));
//        System.out.println(LunarSolarConverterUtil.lunarYearToGanZhi(2100));
